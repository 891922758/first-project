...
for(int i=0;i<L;i++){
	//update U1 a*r1;
	cublasSgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_N,a,r2,b,&alpha,d_X,a,a*b,d_U2,b,0,&beta,d_A1,a,a*r2,c);
	cublasSgemm(handle,CUBLAS_OP_N,CUBLAS_OP_N,a*r2,r3,c,&alpha,d_A1,a*r2,d_U3,c,&beta,d_A2,a*r2);
	cublasSgemm(handle,CUBLAS_OP_N,CUBLAS_OP_T,a,a,r2*r3,&alpha,d_A2,a,d_A2,a,&beta,d_AA2,a);
	// syevd for U1
	cusolverDnSsyevdx_bufferSize(cusolverH,jobz,range,uplo,a,d_AA2,a,0.0,1e06,1,a,&meig1,d_W1,&lwork);
	cudaMalloc((void**)&d_work,sizeof(dt)*lwork);
	cusolverDnSsyevdx(cusolverH,jobz,range,uplo,a,d_AA2,a,0.0,1e06,1,a,&meig1,d_W1,d_work,lwork,devInfo);
	cudaMemcpy(&infogpu,devInfo,sizeof(int),cudaMemcpyDeviceToHost);
//	cout<<infogpu<<endl;
	cudaDeviceSynchronize();
	d_U1=d_AA2+(a-r1)*a; 
	...
	//update U2 b*r2
	cublasSgemm(handle,CUBLAS_OP_T,CUBLAS_OP_N,r1,b*c,a,&alpha,d_U1,a,d_X,a,&beta,d_B1,r1);
	cublasSgemm(handle,CUBLAS_OP_N,CUBLAS_OP_N,r1*b,r3,c,&alpha,d_B1,r1*b,d_U3,c,&beta,d_B2,r1*b);
	// d_B2 r1*b*r3 den r1*r1  = b*r1*r3 =>b*c
	cublasSgemmStridedBatched(handle,CUBLAS_OP_T,CUBLAS_OP_N,b,r1,r1,&alpha,d_B2,r1,r1*b,d_R1den,r1,0,&beta,d_B2T,b,b*r1,r3);
	cublasSgemm(handle,CUBLAS_OP_N,CUBLAS_OP_T,b,b,r1*r3,&alpha,d_B2T,b,d_B2T,b,&beta,d_BB2,b);
	// syevd for U2
	cusolverDnSsyevdx_bufferSize(cusolverH,jobz,range,uplo,b,d_BB2,b,0.0,1e06,1,b,&meig2,d_W2,&lwork);
	cudaMalloc((void**)&d_work,sizeof(dt)*lwork);
	cusolverDnSsyevdx(cusolverH,jobz,range,uplo,b,d_BB2,b,0.0,1e06,1,b,&meig2,d_W2,d_work,lwork,devInfo);
	cudaMemcpy(&infogpu,devInfo,sizeof(int),cudaMemcpyDeviceToHost);
//	cout<<infogpu<<endl;
	cudaDeviceSynchronize();
	d_U2=d_BB2+(b-r2)*b; 
	...
	//update U3 c*r3
	cublasSgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_N,a,r2,b,&alpha,d_X,a,a*b,d_U2,b,0,&beta,d_C1,a,a*r2,c);
	cublasSgemm(handle,CUBLAS_OP_T,CUBLAS_OP_N,r1,r2*c,a,&alpha,d_U1,a,d_C1,a,&beta,d_C2,r1);
	cublasSgemm(handle,CUBLAS_OP_T,CUBLAS_OP_N,c,c,r2*r1,&alpha,d_C2,r1*r2,d_C2,r1*r2,&beta,d_CC2,c);
	// syevd for U3
	cusolverDnSsyevdx_bufferSize(cusolverH,jobz,range,uplo,c,d_CC2,c,0.0,1e06,1,c,&meig3,d_W3,&lwork);
	cudaMalloc((void**)&d_work,sizeof(dt)*lwork);
	cusolverDnSsyevdx(cusolverH,jobz,range,uplo,c,d_CC2,c,0.0,1e06,1,c,&meig3,d_W3,d_work,lwork,devInfo);
	cudaMemcpy(&infogpu,devInfo,sizeof(int),cudaMemcpyDeviceToHost);
//	cout<<infogpu<<endl;
	cudaDeviceSynchronize();
	d_U3=d_CC2+(c-r3)*c; 
	...
	cudaDeviceSynchronize();
/*
// X1=A*G1*(CxB)'  a*r1  r1*(r2*r3)= a*(r2*r3)   (b*c)*(r2*r3) 
	dt *d_temp1,*d_temp2,*d_T;
	cudaMalloc((void**)&d_temp1,sizeof(dt)*a*r2*r3);
	cudaMalloc((void**)&d_temp2,sizeof(dt)*b*c*r2*r3);
	cudaMalloc((void**)&d_T,sizeof(dt)*a*b*c);
	cublasSgemm(handle,CUBLAS_OP_N,CUBLAS_OP_N,a,r2*r3,r1,&alpha,d_U1,a,d_core,r1,&beta,d_temp1,a);
	// compute temp2 =CxB (b*c)*(r2*r3)
	for(int i = 0;i<r3;i++){
		cublasSgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,b,c,1,&alpha,d_U2,b,b,d_U3+i*c,c,0,&beta,d_temp2+i*b*c*r2,b,b*c,r2);
	}

	cublasSgemm(handle,CUBLAS_OP_N,CUBLAS_OP_T,a,b*c,r2*r3,&alpha,d_temp1,a,d_temp2,b*c,&beta,d_T,a);
	cudaDeviceSynchronize();

	cublasSaxpy(handle,a*b*c,&alpha1,d_X,1,d_T,1);
	cublasSnrm2(handle,a*b*c,d_T,1,&sh);
