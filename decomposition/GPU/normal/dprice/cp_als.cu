...
// we assume they all store as column
	...
	dim3 block2((a*b*c+512-1)/512,1,1); //for X2'
...
// we compute kr(dot) product
	cublasDgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,b,c,1,&alpha,d_B,b,b,d_C,c,c,&beta,d_CkrB,b,b*c,r);
...
// compute (CkrB)'*X1'
	...
	cublasDgemm(handle,CUBLAS_OP_T,CUBLAS_OP_T,r,a,b*c,&alpha,d_CkrB,b*c,d_X,a,&beta,d_At_r,r);
...
// update B
	cublasDgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,a,c,1,&alpha,d_A,a,a,d_C,c,c,&beta,d_CkrA,a,a*c,r);
...
// compute (CkrA)'*X2', we have used batch matrix pro to get X2 
	...
	cublasDgemm(handle,CUBLAS_OP_T,CUBLAS_OP_T,r,b,a*c,&alpha,d_CkrA,a*c,d_X2,b,&beta,d_Bt_r,r);
...
// compute (BkrA)'*X3', as Ct_r 
	...
	cublasDgemm(handle,CUBLAS_OP_T,CUBLAS_OP_N,r,c,a*b,&alpha,d_BkrA,a*b,d_X,a*b,&beta,d_Ct_r,r);
...
// we tanspose C' to C in d_C is c*r
	cublasDgeam(handle,CUBLAS_OP_T,CUBLAS_OP_N,c,r,&alpha,d_Ct_r,r,&beta,d_C,c,d_C,c);
//	cout<<"C"<<endl; printTensor(d_C,c,r,1);
	
	// recover to X3' which is same to X
	// X3'= (BkrA)*C' 
	cublasDgemm(handle,CUBLAS_OP_N,CUBLAS_OP_T,a*b,c,r,&alpha,d_BkrA,a*b,d_C,c,&beta,d_rec,a*b);
