...
void gencpTensor(dt *T,int a,int b,int c,int r){
	...
	cublasCreate(&handle);
	cublasDgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,b,c,1,&alpha,d_BB,b,b,d_CC,c,c,&beta,d_CKRB,b,b*c,r);
	cublasDgemm(handle,CUBLAS_OP_N,CUBLAS_OP_T,a,b*c,r,&alpha,d_AA,a,d_CKRB,b*c,&beta,d_T,a);
...
void gencpTensor4(dt *T,int a,int b,int c,int d,int r){
	...
	cublasCreate(&handle);
	cublasDgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,b,c,1,&alpha,d_BB,b,b,d_CC,c,c,&beta,d_CKRB,b,b*c,r);
	cublasDgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,b*c,d,1,&alpha,d_CKRB,b*c,b*c,d_DD,d,d,&beta,d_kr,b*c,b*c*d,r);
	cublasDgemm(handle,CUBLAS_OP_N,CUBLAS_OP_T,a,b*c*d,r,&alpha,d_AA,a,d_kr,b*c*d,&beta,d_T,a);
...
/*	dt *d_temp1,*d_temp2;
	...
	cublasSgemm(handle,CUBLAS_OP_N,CUBLAS_OP_N,a,r2*r3,r1,&alpha,d_A,a,d_G,r1,&beta,d_temp1,a);
	// compute temp2 =CxB (b*c)*(r2*r3)
	for(int i = 0;i<r3;i++){
		cublasSgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,b,c,1,&alpha,d_B,b,b,d_C+i*c,c,0,&beta,d_temp2+i*b*c*r2,b,b*c,r2);
	}
...
*/	
	...
	cublasDgemm(handle,CUBLAS_OP_N,CUBLAS_OP_N,a,r2*r3,r1,&alpha,d_A,a,d_G,r1,&beta,d_coreU1,a);
	cublasDgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,a,b,r2,&alpha,d_coreU1,a,a*r2,d_B,b,0,&beta,d_coreU1U2,a,a*b,r3);
	//cout<<"coreU1U2"<<endl; printTensor(d_coreU1U2,a,b,r3);
	//a*b*r3 c*r3 rec a*b*c
	cublasDgemm(handle,CUBLAS_OP_N,CUBLAS_OP_T,a*b,c,r3,&alpha,d_coreU1U2,a*b,d_C,c,&beta,d_T,a*b);
	cudaMemcpyAsync(T,d_T,sizeof(dt)*a*b*c,cudaMemcpyDeviceToHost,0);
	cudaDeviceSynchronize();
...
// X1=A*G1*(CxB)'  a*r1  r1*(r2*r3)= a*(r2*r3)   (b*c)*(r2*r3) 
	...
	cublasHandle_t handle;
	cublasCreate(&handle);
	cublasDgemm(handle,CUBLAS_OP_N,CUBLAS_OP_N,a,r2*r3,r1,&alpha,d_A,a,d_G,r1,&beta,d_temp1,a);
	// compute temp2 =CxB (b*c)*(r2*r3)
	for(int i = 0;i<r3;i++){
		cublasDgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,b,c,1,&alpha,d_B,b,b,d_C+i*c,c,0,&beta,d_temp2+i*b*c*r2,b,b*c,r2);
	}

	cublasDgemm(handle,CUBLAS_OP_N,CUBLAS_OP_T,a,b*c,r2*r3,&alpha,d_temp1,a,d_temp2,b*c,&beta,d_T,a);
	cudaMemcpyAsync(T,d_T,sizeof(dt)*a*b*c,cudaMemcpyDeviceToHost,0);
	cudaDeviceSynchronize();
	cout<<"X"<<endl; printTensor(d_T,a,b,c);
...
void gentuTensor4(dt *T,int a,int b,int c,int d,int r1,int r2,int r3,int r4){
	...
	cublasDgemm(handle,CUBLAS_OP_N,CUBLAS_OP_N,a,r2*r3*r4,r1,&alpha,d_A,a,d_G,r1,&beta,d_coreU1,a);
	cublasDgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,a,b,r2,&alpha,d_coreU1,a,a*r2,d_B,b,0,&beta,d_coreU1U2,a,a*b,r3*r4);
	cublasDgemmStridedBatched(handle,CUBLAS_OP_N,CUBLAS_OP_T,a*b,c,r3,&alpha,d_coreU1U2,a*b,a*b*r3,d_C,c,0,&beta,d_coreU1U2U3,a*b,a*b*c,r4);
	cublasDgemm(handle,CUBLAS_OP_N,CUBLAS_OP_T,a*b*c,d,r4,&alpha,d_coreU1U2U3,a*b*c,d_D,d,&beta,d_T,a*b*c);
	cudaMemcpyAsync(T,d_T,sizeof(dt)*a*b*c*d,cudaMemcpyDeviceToHost,0);
	cudaDeviceSynchronize();
